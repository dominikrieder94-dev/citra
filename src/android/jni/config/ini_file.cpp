...
void IniFile::ParseLine(const std::string& line, std::string* keyOut, std::string* valueOut) {
    ...
    if (firstEquals != std::string::npos) {
        // Yes, a valid line!
        *keyOut = Common::StripSpaces(line.substr(0, firstEquals));

        if (valueOut) {
            *valueOut = Common::StripQuotes(
                Common::StripSpaces(line.substr(firstEquals + 1, std::string::npos)));
        }
    }
}
...
bool IniFile::Section::GetLines(std::vector<std::string>* lines, const bool remove_comments) const {
    for (const std::string& line : m_lines) {
        std::string stripped_line = Common::StripSpaces(line);
        ...
        if (remove_comments) {
            ...
            if (commentPos != std::string::npos) {
                stripped_line = Common::StripSpaces(stripped_line.substr(0, commentPos));
            }
        }
