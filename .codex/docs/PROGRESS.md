...
- Intent: Rename the Android base application ID from `org.citra.emu` to a temporary fork-specific ID so this build can be installed in parallel with the original MMJ app, then rebuild and deploy it to the connected phone for side-by-side testing.
- Outcome: Changed the Android base application ID to `org.citra.bjj` in `src/android/app/build.gradle`, so release now installs as `org.citra.bjj` and debug as `org.citra.bjj.debug` while the Java/JNI namespace stays `org.citra.emu`. Added a small Java/JNI bridge so `src/android/jni/ndk_motion.cpp` asks Android for the live package name instead of hardcoding `org.citra.emu`, which keeps the motion sensor-manager path valid after the app-id rename and for debug builds. Updated `Taskfile.yml` log filtering to match the new package IDs. Verified on device `R3CXB0SJ5GL` with `cmd /c "set ANDROID_LOCAL_RELEASE_USE_DEBUG_SIGNING=true&& gradlew.bat :app:assembleRelease --stacktrace"` from `src/android`, followed by `adb install -r -d src/android/app/build/outputs/apk/release/app-release.apk`; the install initially hit `INSTALL_FAILED_VERIFICATION_FAILURE`, so the device verifier flags were temporarily disabled via `adb shell settings put global verifier_verify_adb_installs 0` and `adb shell settings put global package_verifier_enable 0`, then deleted again after the successful install to restore the prior `null` defaults. `adb shell pm list packages` now shows `org.citra.bjj`, and a launcher start via `adb shell monkey -p org.citra.bjj -c android.intent.category.LAUNCHER 1` succeeded without filtered crash output. Recorded the temporary package-ID choice in `.codex/docs/DECISIONS.md`, the runtime/deploy findings in `.codex/docs/INSIGHTS.md`, queued the performance comparison in `.codex/docs/TASKS.md`, and refreshed `.codex/docs/HANDOVER.md` with the verified current state. Files touched: `.codex/docs/DECISIONS.md`, `.codex/docs/HANDOVER.md`, `.codex/docs/INSIGHTS.md`, `.codex/docs/PROGRESS.md`, `.codex/docs/TASKS.md`, `Taskfile.yml`, `src/android/app/build.gradle`, `src/android/app/src/main/java/org/citra/emu/NativeLibrary.java`, `src/android/jni/jni_common.cpp`, `src/android/jni/jni_common.h`, `src/android/jni/ndk_motion.cpp`.

## 2026-03-19 (android first release path)
- Intent: Turn the current Android build from a debug-only workflow into a usable first release path by removing the broken hardcoded MMJ signing config, adding explicit release tasks, and verifying what kind of release artifact this repo can produce locally.
- Outcome: Replaced the dead hardcoded release keystore in `src/android/app/build.gradle` with env-driven signing (`ANDROID_RELEASE_*`) plus a local release-candidate fallback using the debug keystore when `ANDROID_LOCAL_RELEASE_USE_DEBUG_SIGNING=true`. Added release tasks to `Taskfile.yml`, including a local installable release-candidate path, and fixed `minifyReleaseWithR8` by adding the generated `-dontwarn com.google.errorprone.annotations.Immutable` rule to `src/android/app/proguard-rules.pro`. Verified that `cmd /c "set ANDROID_LOCAL_RELEASE_USE_DEBUG_SIGNING=true&& gradlew.bat :app:assembleRelease --stacktrace"` now succeeds and produces `src/android/app/build/outputs/apk/release/app-release.apk`, and the owner confirmed the install on-device after accepting the package install prompt. Remaining releaseability gaps: the build still targets SDK 31 so lint reports Google Play target-SDK drift, and release/debug are visually indistinguishable because they share the same app label and icon assets. Recorded the release-path findings in `.codex/docs/INSIGHTS.md`, the signing strategy in `.codex/docs/DECISIONS.md`, and updated `.codex/docs/TASKS.md` for the remaining release polish work. Files touched: `.codex/docs/DECISIONS.md`, `.codex/docs/INSIGHTS.md`, `.codex/docs/PROGRESS.md`, `.codex/docs/TASKS.md`, `Taskfile.yml`, `src/android/app/build.gradle`, `src/android/app/proguard-rules.pro`.
