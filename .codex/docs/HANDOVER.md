...
- Verified state:
  ...
  - The Android repo now has a usable release path again:
    ...
    - explicit signing works through `ANDROID_RELEASE_STORE_FILE`, `ANDROID_RELEASE_STORE_PASSWORD`, `ANDROID_RELEASE_KEY_ALIAS`, and `ANDROID_RELEASE_KEY_PASSWORD`
    - local release-candidate builds work through `ANDROID_LOCAL_RELEASE_USE_DEBUG_SIGNING=true`
  - Local release-candidate verification on 2026-03-19:
    - `cmd /c "set ANDROID_LOCAL_RELEASE_USE_DEBUG_SIGNING=true&& gradlew.bat :app:assembleRelease --stacktrace"` succeeded from `src/android`
    - output APK: `src/android/app/build/outputs/apk/release/app-release.apk`
    - the owner confirmed the release APK installed successfully on `R3CXB0SJ5GL` after accepting the install prompt
  - Release-only shrinker failure is addressed:
    - `src/android/app/proguard-rules.pro` now includes `-dontwarn com.google.errorprone.annotations.Immutable`
    - this suppresses the Tink-related R8 release failure that blocked `minifyReleaseWithR8`
  - Remaining releaseability gaps:
    - debug and release are visually indistinguishable on-device because they currently share the same label and launcher assets
    - the app still targets SDK 31, so lint reports it as below current Google Play expectations
  - Operational gotcha:
    - `task build-release-apk` currently runs through `setup-android` and can still fail inside `git submodule update --init --recursive` because of a nested dynarmic/zydis config permission issue
    - the direct Gradle release command from `src/android` is the verified working path for now
- First next steps:
  1. Make debug and release visually distinct on-device, for example `Citra BJJ Debug` for the debug label and/or a debug-only launcher badge.
  2. Decide whether to fix the recursive submodule permission issue in the task-based release workflow or document direct `gradlew` release builds as the temporary contract.
  3. Raise Android target SDK and related release settings to move from local release candidate toward a real publishable build.
...
- Verified state:
  ...
  - Follow-on architecture decision captured in `.codex/docs/DECISIONS.md`: owner-forked submodules are first-class compatibility contracts and will remain pinned until upstream equivalence is validated.
